...
import SmallSpinner from "../Spineer/SmallSpinner";

const Currency = ({ currencyDetails, setCurrencyDetails, countryDetails }) => {
  ...
  const handleCurrencyConvert = (e) => {
    ...
    const myHeaders = {
      apikey: `${process.env.REACT_APP_exchangerates_apikey}`,
    };
    axios
      .get(
        `https://api.apilayer.com/exchangerates_data/convert?to=USD&from=${countryDetails?.currency_code}&amount=${amount}`,
        { headers: myHeaders }
      )
      .then((response) => {
      ...
      <div>
        <div className="w-full max-w-md overflow-hidden rounded-lg bg-gray-800/90 shadow-lg">
          <form className="p-5" onSubmit={handleCurrencyConvert}>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    disabled
                  >
                    <option value="USD">{countryDetails?.currency_code}</option>
                  </select>
                </div>
              </div>
              <div>
            ...
            <button
              className="my-2 rounded-lg bg-gradient-to-r from-blue-600 to-blue-800 px-4 py-2 font-medium text-white focus:outline-none"
              type="submit"
            >
              Convert
            </button>
